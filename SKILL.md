...
| 底/字 | 暖墨/石墨近黑 + 骨白（颗粒+顶光） | 纯灰阶三档 | 奶油底 + 深墨 |
| 感觉 | 影像说话、跟手叙事 | 克制、无装饰、信息密度高 | 纸感、衬线、可长读 |

**默认 token（方向 A · 石墨香槟，直接抄；另有「墨·胭脂」「深岩·青瓷」两套见参考文件）**

| Token | 值 | 用途 |
|---|---|---|
| `--bg` / `--bg-2` / `--bg-3` | `#0C0C0D` / `#141415` / `#1C1C1E` | 页面 / 分区 / 抬升面（只差亮度） |
| `--fg` | `#EDEAE4` | 骨白：标题与主文字 |
| `--fg-2` | `rgba(237,234,228,.74)` | 正文/导语（同色降透明，不换色） |
| `--fg-3` | `rgba(237,234,228,.56)` | 元信息/编号 |
| `--rule` | `rgba(237,234,228,.11)` | 1px 分隔线 |
| `--accent` | `#C9A96E` 香槟 | **全页唯一点缀**：圆点、进度线、编号、按钮悬停 |
| `--live` | `#D0574A` 胭脂 | 只给 REC / 直播中 状态点 |

**暗色质感四件套**（缺一件就会显得 low）：带温度的近黑底（不是藏青、不是纯黑）+ 骨白文字同色温透明度阶梯 + 胶片颗粒与顶光暗角 + 一个偏沉的点缀色。v1 的藏青底 + 冷蓝白 + 马卡龙粉/薄荷/薰衣草已废弃，原因见 `references/aesthetic-directions.md` A 节。

方向 C（亮色暖调）token：`--bg #F7F4EF`／`--ink #1A1A1A`／`--muted #6E635B`／`--accent #E89B5A`。

**形状与节奏**：圆角只用两档（媒体 `4px`、内容件 `8–10px` / `999px` 胶囊），**不要** 16–24px 全套大圆角；阴影克制（深色底下用亮度分层，不用辉光）；间距用一套数：`8 / 14 / 22 / 34 / 56 / 88`；正文 17px/1.8（手机）、18px/1.7（PC）；中文标题行高压到 1.1–1.2。
